...
def process_and_segment_audio(filepath, target_sr, duration_s, error_log_file=None):
...
def main():
    ...
    error_log_path = BASE_SEGMENTS_DIR / "excluded_files.log"
    print(f"Logging any excluded files to: {error_log_path}")
        ...
        if esc50_meta_path.exists():
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), desc="ESC-50 Base Prep"):
                if row['category'] in ESC50_MAP:
                    ...
                    audio = process_and_segment_audio(audio_path, SAMPLE_RATE, DURATION_S, error_log)
        ...
        if us8k_meta_path.exists():
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), desc="UrbanSound8K Base Prep"):
                if row['class'] in URBANSOUND8K_MAP:
                    ...
                    audio = process_and_segment_audio(path, SAMPLE_RATE, DURATION_S, error_log)
                    if audio is not None:
